...
func (repo *gatewayPostgreRepository) GetGatewayBYId(gatewayId string) error {
...
func (repo *gatewayPostgreRepository) GetGatewaysByTenantId(tenantId string) ([]Gateway, error) {
	var gatewayEntities []GatewayEntity
	err := repo.db.
		Where("tenant_id = ?", tenantId).
		Find(&gatewayEntities).
		Error
	if err != nil {
		return nil, err
	}

	gateways := make([]Gateway, len(gatewayEntities))
	for i, entity := range gatewayEntities {
...
func (repo *gatewayPostgreRepository) GetAllGateways() ([]Gateway, error) {
	var gatewayEntities []GatewayEntity
	err := repo.db.Find(&gatewayEntities).Error
	if err != nil {
		return nil, err
	}
