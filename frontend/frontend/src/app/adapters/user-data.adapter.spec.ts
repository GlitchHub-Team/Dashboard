...
describe('UserDataAdapter', () => {
  ...
  it('should adapt a raw user config to a user', () => {
    const raw = { id: '1', username: 'admin', email: 'admin@test.com', role: UserRole.TENANT_ADMIN, tenantId: 'tenant-1' } as unknown as RawUserConfig;
    const expected: User = { id: '1', username: 'admin', email: 'admin@test.com', role: UserRole.TENANT_ADMIN, tenantId: 'tenant-1' };
  ...
  it('should handle missing or null fields when adapting', () => {
    ...
    const raw = { id: '2', username: null, email: null } as unknown as RawUserConfig; 
    // Adatta i valori expected ai default che hai effettivamente impostato nel tuo UserDataAdapter
    const expected: User = { id: '2', username: '', email: '', role: undefined as unknown as UserRole, tenantId: '' }; 
  ...
  it('should adapt an array of raw user configs', () => {
    const rawArray = [
      { id: '1', username: 'admin', email: 'admin@test.com', role: UserRole.TENANT_ADMIN, tenantId: 'tenant-1' },
      { id: '2', username: 'user', email: 'user@test.com', role: UserRole.TENANT_USER, tenantId: 'tenant-1' }
