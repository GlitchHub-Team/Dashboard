...
describe('GatewayCommandApiClientService', () => {
  ...
  describe('commissionGateway', () => {
    it('should send POST request with gateway id in the URL and tenant id in the body', () => {
      service.commissionGateway('gw-1', 'tenant-1').subscribe((gateway) => {
        expect(gateway).toEqual(mockGateway);
      });

      const req = httpMock.expectOne(`${apiUrl}/gateway/gw-1/commission`);
      expect(req.request.method).toBe('POST');
      expect(req.request.body).toEqual({ tenant_id: 'tenant-1' });
